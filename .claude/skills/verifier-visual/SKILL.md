...
- **Do not copy the load.** `ui_shell_fixture.lua` writes a snapshot and captures both sides of
  ...
  `verify.econ_step` never advanced, so the activity fog ages everything as stale — NR-590) and
  the comms dock comes back empty (the chat log is not in the save envelope — NR-591). Until those
