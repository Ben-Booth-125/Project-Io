...
## Latest releases — v0.1.21, v0.1.22, v0.1.23 and v0.1.24

**v0.1.24 — the world gets a history, and the instruments learn to read it.** The pre-campaign past stops being one ancient pass and becomes an arc the player inherits: the Empires phase now runs its real 400 BCE → 1200 CE instead of the 400-year slice it had been silently running, and a new Exploration age carries it 1200 → 1660 with per-polity treasuries, treaties that expire, colonies that answer to an overlord, and fleets that decay when nobody pays for them. Goods began to cross water for reasons the simulation can state — trade as a want met by throughput, opened only by a trade-access clause — and conflict finally pointed outward, displacement moving from a median 0.08 to 1.33 across sixteen seeds. Four pre-game technology trees were written under one grammar, and the planetary ground was rebuilt to read as terrain rather than a grid. **But the release's subject is what the measurements said once the instruments were aimed properly.** A whole sprint's figures turned out to describe a world nobody plays: the sweeps built from struct defaults while the game loads its Lua config, so seed 0 fought **6,479 battles in the sweep and 9,928 in the game**. Nothing in the scorer reads its own purse — with the saturation caps removed the median polity ends the span holding 211 against 1.26M before, and one polity-round in seven cannot pay its army bill. And the world's diplomacy is a seal rather than a deterrent: 1,250 first contacts by 1660, and not one of 492 new pairs holding a non-aggression pact. All three are recorded rather than quietly fixed.

**v0.1.23 — the ledgers get read properly, and two recorded causes turn out to be wrong.** The six surfaces the last batch never reached were reviewed; three were rebuilt. But the release's subject is what re-measuring found. The History ledger's Ages view had produced **no frame in nineteen minutes**, and the filed cause said the simulation was slow. It was not: the view was asking for 1960 decision rounds where generation runs 100, on a span lying entirely after the era the world has. Fixing that made it render — and it still reported zero battles and zero conquests, because it replayed the world's history starting from that history's own ending. Generation now records what happened and the view replays it: four battles, one conquest, and eight seconds where it had been minutes. The same pattern caught a red harness nobody owned, which turned out to be an assertion comparing a float against a double and demanding they match exactly. And the retired mercenary contract, left "dormant" a release ago, was still running every tick and still telling players about contracts they could not open — now removed outright, along with the only authored predicate content in the game, which is recorded rather than quietly absorbed.
