...
// Deliberately NO urgency/importance fields (Ben, 2026-08-02): those are scored
// by the live AI at decision time on a 2D urgency x importance map, against
// world state. The dictionary supplies meaning; the policy supplies priority.
const ids = new Set();
for (const e of entries) {
...
// The compact index (Ben, 2026-08-02): a language agent must not need the whole
// store in context. It holds THIS in its prompt — one line per action — and
// pulls full entries on demand via tools/session/actions_query.js. Derived,
// regenerated with the mirror; never hand-edited.
const INDEX_OUT = path.join(ROOT, 'docs', 'ai', 'ACTIONS_INDEX.json');
const index = {
  _schema: 'Generated by render_actions.js from ACTIONS.json — do not hand-edit. One row per action: [id, surface]. The agent-facing table of contents: hold this in context, fetch full entries by id via tools/session/actions_query.js. No urgency/importance here — the live AI scores those at decision time.',
  actions: entries.map((e) => [e.id, e.surface]),
};
fs.writeFileSync(INDEX_OUT, JSON.stringify(index, null, 1) + '\n');

console.log(`render_actions: ${entries.length} entries -> ${path.relative(ROOT, OUT)} + ${path.relative(ROOT, INDEX_OUT)}`);
