...
// hand-built finding), not on the economy; plus R5, which fails on a basket
// naming a good no market prices. R5 is content, not magnitude: it asserts no
...
//   R5  BL-652: FAILS if a demand basket names a good NO market prices. The one
...
void print_band(const band_result& b)
{
    ...
    std::printf("  a basket names it, NO market prices it (BL-652, asserted by R5): %s\n",
...
int main(int argc, char** argv)
{
    ...
    std::printf("demand_census — BL-649, requirement group `demand-census` R1-R5\n");
    ...
    for (const band_result& b : results)
    {
        ...
        // --- R5 (BL-652) — THE ONE MAGNITUDE-FREE ROW THAT FAILS -------------
        ...
        std::snprintf(msg, sizeof msg,
                      ...
                      b.basket_unpriced.empty() ? "none do"
                                                : join(b.basket_unpriced).c_str());
        check(b.basket_unpriced.empty(), "R5", msg);
    }
