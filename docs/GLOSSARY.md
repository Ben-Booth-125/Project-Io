...
**Syndicate**
*(Entry added 2026-08-21 — BL-522, the ownership tier. Word chosen on Ben's behalf, NR-479.)* The **ownership** actor: it holds equity in operating corporations, allocates capital, and lives off the profit. It owns no buildings and runs no recipes — that is what distinguishes it from a **Corporation**, which is the operating firm and keeps that meaning unchanged. **Seven syndicates** (the player one of them — Ben, NR-475) to roughly 88 corporations. **Equity confers operating control above a > 50% majority (Ben, 2026-08-21, NR-473, overturning an earlier call that it never did):** a corporation's **controlling holder** is the syndicate above that line, derived from the equity relation and never stored as a flag. A corporation whose controlling holder is the player's syndicate **is** the player's corp for the standing rule and is never auto-acted on; every other corporation runs itself on its own `corp_ai` scorer. **Below the threshold a holding is purely financial** — a claim on profit, no say in the build order — so crossing the majority line is a real strategic objective, and control costs attention, which is the tier's central trade-off. *Designed, not built* — BL-522, priority A, version goal v0.1.23 (proposed, NR-476); the mechanism lives in BL-523 (equity data model) through BL-528 (portfolio ledger). See `docs/development/backlog.json`.
