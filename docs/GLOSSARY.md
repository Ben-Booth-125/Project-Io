...
**Province**
The **locality cell** — a small area grouping of roughly 3–5 tiles, below the region and above the tile. Its purpose is to price locality *before* the market does: goods sourced inside one province are cheaper to the buyer than goods hauled in from outside it. **Designed 2026-08-18 (Ben), not built** — no province type, no tile→province mapping and no discount exists yet. It is deliberately a *different object* from the region: a region is large, irregular, historically placed and politically owned, while a province is small, uniform and purely spatial. See BL-464 (Logistic Points) for the neighbouring throughput work, and NR-342 for the open duplication question — convoy haulage cost already prices distance, so the province discount must earn its place against that rather than restate it.
