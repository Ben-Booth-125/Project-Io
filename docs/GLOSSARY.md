...
**Metropole / colony**
In the colonial era (`docs/generation/COLONIAL_ERA.md`), the **metropole** is the polity that claimed a province across water and the **colony** is the province claimed — by **purchase** (bought for trade, its culture shares kept) or by **conquest** (taken, its people digested). Both are polity verbs of the generation sim, never campaign-era actions. The colony grows the endemic good; the metropole wants it. What the pair leaves on the campaign map is a **sea lane** and, at setup, a taste, a treasury and a tariff — never a preferred-seller relationship in the order book (superseded, Ben, 2026-09-09).

**Sea lane**
A tier on water tiles that discounts the sea-leg traversal cost, stamped from the colonial era's record of the crossings it actually made — the water analogue of an ancient road (`docs/economy/LOGISTICS.md` § 4b). It is the **colonial tie** as a force on the map: read by convoys, reach and placement alike because traversal cost is one weight function. Not a shipping route in play, and not a road — land is untouched.
...
**Era**
A named phase in the game's industrial arc, defined by the accessible territory, available buildings, dominant strategic challenge, and **the one catastrophe the player is playing to avoid**. The ladder is 1-based and each number matches its catastrophe (renumbered 2026-08-31): **Era 1** (Terrestrial — nuclear war) transitions to **Era 2** (Early Space — climate collapse) by meeting an explicit gate condition. Note this is the *document's* Era axis; `era_band` and `condition_subject::era` are separate axes that did not renumber with it. See `docs/economy/ERAS.md` § The ladder and `docs/CONCEPT.md` § Eras.
