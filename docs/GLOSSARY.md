...
**Sea lane**
A tier on water tiles that discounts the sea-leg traversal cost, stamped from the exploration age's sea-leg table — the crossings it actually made (`docs/generation/EXPLORATION.md` § The colonial tie is a sea lane, and the map reads it) — the water analogue of an ancient road (`docs/economy/LOGISTICS.md` § 4b). It is the **colonial tie** as a force on the map: read by convoys, reach and placement alike because traversal cost is one weight function. Not a shipping route in play, and not a road — land is untouched.
