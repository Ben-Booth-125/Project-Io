...
The live product is the **industrial arc** (Ben, 2026-09-16, NR-807): the campaign epoch is **1960**, and generation runs 2400 BCE → 1960 to produce the world it opens on — an ancient span that settles the ground and its peoples, then an exploration span and an industrial one that leave nations, wealth and demand behind. `era_band_for_epoch` flips to the industrial band at 1700, so the arc the generator runs and the arc the player meets are the same arc. The Era ladder below **begins at Era 1 — Terrestrial**, which is that epoch; the Eras after it are the **space arc**, the same engine further along, intended as downloadable content (ROADMAP § The two arcs). All of them share the core loop and the design test above.

The 0 CE ancient arc this paragraph named until 2026-09-16 is **superseded as the live product**, not parked: the ancient span survives as *generation* — the millennia the history sim runs before the campaign opens — rather than as the century the player plays in. **Who the player is did not move with the arc.** The mercenary company was ruled on 2026-08-12 (NR-177) for a reason that names no century — a company reaches force directly where a corporation's levers are all economic — so § Player identity stands as written until it is ruled on in its own right.
