...
**Expected output.** `is_player` and `world::player_entity` re-point onto `corp` (every other corporation's flag cleared), the player-scoped balance series is rebuilt from that corporation's own filed returns, and play opens framed on its holdings. Nothing else moves: the same world and the same pick give the same player and the same state hash (reproducible from (seed, pick); scripts/verify/seat_pick.lua S5, tools/verify/seat_pick_check.js). world::state_hash does not fold the seat itself, so the seat is read as world::player_entity beside it. Picking the corporation the weighted draw would have chosen gives exactly the state the draw gives. The log prints `[seat] picked <id> (take_seat)`; a path with no player to ask prints `[seat] drawn <id> - <why>`.
