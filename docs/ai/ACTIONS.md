...
*144 entries — 27 gameplay · 24 canvas · 15 lens · 45 ledger · 33 chrome.*
...
### `gameplay.accept_offer` — A corp_verb on the corp-command seam (ProjectIo --serve, COMMAND opcode). No in-app press yet — the Contracts ledger (BL-576, a later wave) is the intended surface; this item is the data model and the two verbs only.

**Press.** Over the seam: COMMAND corp=<id> verb=25 order=<mercenary_offer id> counterparty=<nation id> units=<up to 8 of the corp's own unit ids>. Applied through apply_corp_command, which recomputes every precondition itself and mutates nothing on any rejection.

| Arg | Type | Meaning |
|---|---|---|
| `order` | `uint32 (mercenary_offer::id)` | The offer being accepted. 0 is refused (rejected_invalid) — no offer is ever id 0. |
| `counterparty` | `entity_id` | The offer's CLIENT NATION, not a corp — this verb forks the seam's usual corp-counterparty reading. Must name a real entry in world.nations AND match the offer's own client, so an id that merely fits the type is still refused (rejected_invalid) if it names the wrong nation or no nation at all. |
| `units` | `entity_id[8]` | The committed force — the corp's OWN units, named by the caller (unused slots are null_entity/omitted). "The player chooses the force, the contract never does" (CONTRACTS.md Q1): this field is the whole mechanism. At least one unit must be named. |

**Valid when:**
- `order` names a live entry in world.mercenary_offers (rejected_invalid otherwise — including an offer already consumed by a prior accept_offer).
- `counterparty` names a real nation (rejected_invalid otherwise) that is exactly this offer's `client` (rejected_invalid otherwise).
- The offer has not expired: `w.current_econ_tick` is still before the offer's own `deadline` (rejected_invalid otherwise) — accepting a contract that could not possibly complete in time is refused.
- The offer is FULLY escrowed — `offer_escrow >= fee` (rejected_state otherwise): CONTRACTS.md's "the fee this offer's escrow must clear before the contract it names is postable".
- Every non-null `units` slot names a real unit (rejected_invalid otherwise), owned by the acting corp (rejected_not_owner otherwise), and not already committed to another ACTIVE mercenary_contract (rejected_state otherwise — no double-commit).
- At least one unit is named (rejected_invalid on an empty force).

**Expected output.** A new world.mercenary_contracts entry in the `active` state, copying client/template/province/fee/deadline from the offer and accepted_tick from the current econ tick. The deposit (fee × economy.procurement.deposit_fraction) is credited to the acting corp's balance immediately, straight from the offer's already-fully-funded escrow — not a fresh nation-budget draw, since the full fee already left the client nation's treasury while the offer's escrow was filling. The consumed offer is erased from world.mercenary_offers. The named units are locked to this contract: they cannot be disbanded (rejected_state) or committed to a second contract while it stays active, though marching them stays free (BL-573 does not touch unit position or order).

**Reason to select.** The whole reason to hold force at all outside extraction — CONTRACTS.md's income-that-is-not-extraction loop. Weigh the fee against what the objective actually needs: underbidding is the loop's teeth (send too little, lose the fight the tick evaluation pass judges, and both the fee and standing go with it), and every named unit is frozen out of any OTHER contract until this one resolves. Reachable only once an offer exists (BL-572, derive_contract_offers) and only while it is fully escrowed — a nation still filling one has nothing to accept yet.

### `gameplay.abandon_contract` — A corp_verb on the corp-command seam (ProjectIo --serve, COMMAND opcode). No in-app press yet — see accept_offer's own surface note.

**Press.** Over the seam: COMMAND corp=<id> verb=26 order=<mercenary_contract id>. Applied through apply_corp_command.

| Arg | Type | Meaning |
|---|---|---|
| `order` | `uint32 (mercenary_contract::id)` | The contract being abandoned. 0 is refused (rejected_invalid). |

**Valid when:**
- `order` names a live entry in world.mercenary_contracts (rejected_invalid otherwise).
- The acting corp is that contract's `contractor` (rejected_not_owner otherwise) — only the company that took the job can walk away from it.
- That contract is still `active` (rejected_state otherwise) — one already completed, failed, or abandoned has nothing left to abandon.

**Expected output.** The contract's state becomes `abandoned`. No money moves — the deposit paid at accept_offer is not clawed back, and the reserved remainder (fee minus deposit) is simply never disbursed, the same money outcome CONTRACTS.md § Q2 gives a failure. `sentiment_factor_kind::contract_cancelled` is folded (client observes contractor) at a magnitude LESS severe than a failure's `contract_failed` (scripts/economy.lua) — the distinct, lesser penalty CONTRACTS.md's Q2 table names for an honest early exit. The committed units are freed: they may be disbanded or committed to a new contract immediately, with no separate release step.

**Reason to select.** An honest exit from a job that has turned bad, priced cheaper than losing the fight would be. Weigh it the moment the predicate looks unwinnable before the deadline — CONTRACTS.md: "an honest early exit costs less than a rout" — against holding on for the (increasingly unlikely) full fee. There is no partial credit either way; the only question is which of the two lesser penalties to take.
