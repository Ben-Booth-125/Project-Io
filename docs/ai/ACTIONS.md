...
| `type` | `building_type` | Which building — extraction_site, processing_facility, port, launchpad, inland_logistics_hub, military_base, research_institute, schooling, university. Set by which row is pressed. Schooling and university (BL-615) are passive education buildings with no ledger row yet; the seam accepts them and the stratum gate below decides where they may stand. |
...
| `quantity` | `float > 0` | Maximum units offered per quarter, capped each tick by what the corp's (corp, market) pools on that body actually hold — listed from each pool in ascending market id, each into its own market, until the quantity is spent. |
...
**Expected output.** The order is erased from `world::sell_orders` immediately, at no cost; every surviving order keeps its id. From the next economy tick that quantity is no longer listed. Routing consequence, unchanged: if this was the LAST remaining order for that (corp, body, resource) triple, the triple returns to the automatic surplus-selling path — each tick every (corp, market) pool's surplus above its processor reservation auto-sells in that pool's own market at its reference price, with no floor protection. If another order for the same triple still stands, manual control persists. Nothing else changes; already-cleared past sales are untouched. A rejected removal mutates nothing.
...
### `gameplay.dispatch_convoy` — The market Selection card's Dispatch convoy form (BL-607, 2026-08-24) — a resource picker (the market's own tradeable goods), a quantity field and a destination-market picker (every other market, by city name), opened by selecting a market and reached without a nav-rail slot. Issued the same way against the corp-command seam, or over --serve, which needed no new wire key: it reuses subject / counterparty / target / quantity. The Market Ledger's Convoys tab (BL-453) shows the RESULT and carries the Hold press, but composes no dispatch.

**Press.** Select a market (canvas click, or the Market Ledger's row), then in the Selection band's 'Dispatch convoy' section pick a Cargo good, a Quantity and a Destination, and press Dispatch (src/ui/selection_panel.cpp's draw_market_dispatch_form). A rejection (no route, insufficient funds, not enough stock) prints inline under the button rather than failing silently. Context worth carrying: before this verb the logistics layer had NO player verb at all — supply_system.cpp, logistics.cpp, four rendering paths and the only coupling between two markets' prices were entirely automatic, and none of the fifteen prior verbs named a convoy. The verb is the auto-dispatcher's own dispatch body with its shortfall scan removed (supply_system.hpp's price_convoy_leg + commit_convoy), so a convoy you dispatch costs, travels and arrives exactly as one the engine dispatched for a rival. That is asserted, not assumed: tools/verify/convoy_command.cpp R4 compares a player convoy and an auto convoy of the same shape on cost, speed and mode.

| Arg | Type | Meaning |
|---|---|---|
| `source_market` | `entity_id` | The market the cargo leaves, carried in the command's `subject` field. The pool debited is (corp, this market) — goods pools are keyed per market (PRODUCTION.md § Stockpile and output flow), so the stock must sit in THIS market's catchment. An intra-body haul routes from the corp's lowest-id building in this market's catchment (or, if it holds none there, this market's own centre tile) to the destination market's centre tile. Naming the destination as the source is refused (a haul into the market the goods already sit in is no haul). The named market is recorded on the convoy as its source endpoint, which is what the Convoys tab and the canvases draw. |
...
| `quantity` | `float > 0` | Units to haul. Validated as the value that lands in the destination: finite FIRST, then strictly positive, then against what the (corp, source market) pool actually holds. A quantity above the pool is REFUSED, never clamped down to it — a caller hauls the amount it named or none at all, so a silent partial haul can never happen. |
...
- The acting corp's own pool AT the source market holds at least that quantity (rejected_state otherwise). This is also the ownership check: pools are keyed (corp, market), so there is no way to name another corp's stock.
...
- An intra-body leg's source anchor has enough passive Logistic Points left THIS TICK for the leg's distance (rejected_no_lp otherwise; BL-597, LOGISTICS.md § Logistic Points). LP is a CAP, not a second price — no extra credit charge, just admissibility. Space (inter-body) legs are exempt: LP is tile-grounded infrastructure. The pool is shared with active LP (militaries marching this same tick), so heavy march traffic at a contested anchor can genuinely starve this dispatch, and vice versa — a war can flip which side claims the anchor first.

**Expected output.** One `convoy_component` appended to `world::convoys`, carrying a stable nonzero `id` that `hold_convoy` later names (see that entry: the id is TRANSIENT). Cargo leaves the source pool AT DISPATCH, not on arrival — goods in transit are committed (SUPPLY.md § Convoy entity), which is why there is no cancel verb and never will be one. The haul cost is debited from the corp's balance in the same step and recorded on the convoy as `cost_paid`; a space-mode launch also burns one unit of propellant from the source pool. Mode is DERIVED, never named: land, or sea when the intra-body path crosses water, or space between bodies (air is never dispatched). Speed is 1/travel_ticks, and travel_ticks is the terrain-weighted, physically-scaled figure — since 2026-08-12 a long haul genuinely takes several quarters, so a dispatch commits that stock for that many ticks. NOTHING reaches the destination market at dispatch: the cargo credits the destination (corp, market) pool on arrival and reaches pricing through the ordinary auto-surplus path at the next clear — it sells AT the destination, even on a same-body haul. A rejected command mutates nothing at all — asserted against a full world fingerprint, not a spot check (convoy_command.cpp R1).
...
| Arg | Type | Meaning |
|---|---|---|
| `subject` | `entity_id` | The population centre to raze. Demotes to the razed tier (BL-624, razed settlement tier) — population zeroed and the razed state set, but the entity, its name, its tile and the urban ground it stamped (BL-612) all persist. |

**Valid when:**
- `subject` names a real population centre with a tile record (rejected_invalid otherwise).
- `subject` is not already razed (rejected_state — a ruin cannot be razed again).
...
### `gameplay.buy_corporation` — The Acquisitions ledger (nav rail slot 5, and the Company lens's click destination): the Buy press on a PURCHASABLE row. The two groups are the whole affordance — Purchasable rows carry the press, Possible rows show the price and carry none, because a price you cannot meet is something to save toward rather than something refused. Also still a corp_verb, so an agent issues it against the corp-command seam (ProjectIo --serve, COMMAND opcode) without going through the ledger. NOTE FOR A PLANNER: the field is LARGE — measured over twelve seeds of the shipped spawn (tools/verify/acquisition_viability.cpp section C), 88 corporations yield a mean of 81.6 buyable firms, 35.8 of them Purchasable at the moment of reading. A company is publicly_held by construction (BL-678, companies are open); only a corporation can class private or closed and be unpriceable, which is a mean of 5.2 per seed. A mean of 10.6 firms per seed price at exactly ZERO, which is not a bargain - the price floor caught a firm whose debts exceed its book value, and buy_corporation transfers that debt at face value.
...
### `gameplay.take_seat` — The corporation selection canvas at Begin (STARTUP.md § The seat): the ranked list of every specialist beside a map of the home body, a firm's briefing, and its Confirm press. The ONE pre-play entry in this dictionary: the wizard's rounds have none (NR-804), but the pick is a game act (Ben, 2026-09-24) - it decides which corporation the player IS - so an agent must be able to make it. It is a corp_verb (29), but the seat is taken ONCE, BEFORE PLAY: the in-play agent hosts (--serve's COMMAND and the live --host-agent seam) refuse it with rejected_state, and an agent makes the pick at launch with `--seat <corp-id>` (every new-campaign path, --autostart included). NOTE FOR A PLANNER: the list order is the static landscape score (the ground a firm's holdings stand on), and firms below the viability floor are MARKED but offered; a marked firm is a harder opening, never an illegal one.

**Press.** Hover a row to point the map at that firm (its HQ, its works, its home market's catchment) and read its card; press the row to open its briefing; press Confirm. Back (or pressing the same row again) closes the briefing with nothing changed. At launch: `ProjectIo --seat <corp-id>` (the id is the corporation's entity id, as the `[seat]` log line and the blackboard print it).

| Arg | Type | Meaning |
|---|---|---|
| `corp` | `entity_id` | The corporation to become. Must be a SPECIALIST of this world - one of the ranked rows the canvas offers - never a background firm (a company). |

**Valid when:**
- Before play: the validation run is over and no seat has been taken. In play the verb is refused whole (rejected_state) by every agent host, before the seam.
- `corp` names a real corporation (rejected_no_corp otherwise). `--seat` range-checks the id against entity_id's domain before any narrowing and refuses the launch outright on a malformed, negative or out-of-range token - never truncated into some other firm.
- `corp` is a ranked specialist: not a background firm, and one the canvas offers (rejected_invalid otherwise). A firm below the viability floor IS offered and IS pickable.
- A refused pick mutates nothing and seats nobody: a headless run with a refused `--seat` fails rather than falling back to the draw; the interactive path opens the canvas instead.

**Expected output.** `is_player` and `world::player_entity` re-point onto `corp` (every other corporation's flag cleared), the player-scoped balance series is rebuilt from that corporation's own filed returns, and play opens framed on its holdings. Nothing else moves: the same world and the same pick give the same player and the same state hash (reproducible from (seed, pick); scripts/verify/seat_pick.lua S5, tools/verify/seat_pick_check.js). world::state_hash folds the seat (`is_player` / `world::player_entity`), so a different pick on the same world is a different state hash and the seat is asserted from the hash alone; the player id printed beside it is a diagnostic. Picking the corporation the weighted draw would have chosen gives exactly the state the draw gives. The log prints `[seat] picked <id> (take_seat)`; a path with no player to ask prints `[seat] drawn <id> - <why>`.

**Reason to select.** Choose the opening position rather than be dealt one: the card shows what the firm makes and what its home market pays for it, what it holds and owes, and whose law it trades under - so a planner can prefer depth (a processor near population), a debt-free book, or a friendly nation, or knowingly take a marked firm as a harder game.
...
**Expected output.** Each tectonic plate tints a categorical colour from a dedicated ten-colour earthy table at 0.80 opacity (deliberately not the nation wheel — plates are substrate, not identity). Boundary tiles — any neighbour on another plate — get a separate white lift at 0.45, so plate seams read pale. The key explains that pale tiles are boundaries and reports the plate count; a stagnant-lid body says 'one immobile plate' rather than drawing a meaningless single tint. Terrain texture (BL-520) survives this lens at 0.45 strength, with each mark's ink derived from the tile's own lens-tinted fill — so it reads as shading on the lens colour, never as a second, competing colour. HOVER AND SELECTION (BL-660, 2026-08-28): hovering lights the whole tectonic PLATE under the pointer, and clicking selects it and opens the History Ledger. Like the deposit, a plate is not an entity — it travels in ui_state::selected_plate. The History ledger's dedicated tectonic section, describing that plate's convergent (collision) and divergent (rift) boundaries, is BL-660's second half and is NOT yet built: the click currently opens the ledger without aiming it. National borders are NOT drawn while any lens is active (2026-08-28) — neither the inward band nor the coloured frontier rule, and the border's click corridor goes with them. The plain canvas is the only place the national read appears.
...
### `lens.key_toggle` — The lens chrome region's header bar — the minimap's header, top right (BL-602). The bar carries a caret, the key's title, and its row count, e.g. '^ Countries (44)'. Its foot rests on the minimap's top edge and stays there whether the list is open or shut.

**Press.** Single left-click anywhere on the header bar. Only the four COUNT-DRIVEN keys have one — Country, Market, Reach and Supply-routes, whose row list grows with the world. The fixed-height gradient keys (Resource, Scarcity, Population, Industry, Continent) have nothing to overflow and draw open, so they carry no toggle.

**Valid when:**
- One of Country, Market, Reach or Supply-routes must be active — with no lens, or with a gradient-key lens, there is no header bar to press.
- Collapsed by default on every lens switch, so changing lens never throws a forty-row list across the column.
- Toggle rule: the bar shows its own open/shut state, so pressing it while open closes it.

**Expected output.** The row list opens UPWARD out of the minimap's header, filling the right chrome column and stopping one margin below the time panel; a list longer than that space scrolls inside the box (wheel or drag) rather than overrunning it. Long labels wrap rather than widening the box. The header bar itself does not move — that is why the body grows upward from it — so the second press lands on the same pixels as the first. The press does NOT reach the canvas behind the region: the selection is unchanged by opening or closing a key.
...
**Reason to select.** The only surface that prices another corporation, and therefore the only place a whole-firm buyout can be planned rather than merely issued. Expect a LONG list: measured over twelve seeds, a mean of 81.6 firms of eighty-eight are buyable - 35.8 Purchasable and 45.8 Possible. Companies are open by construction (BL-678, companies are open), so ownership class excludes only the handful of rivals that class private or closed, mean 5.2. The field is the world's commercial population, so rank it before reading it rather than reading it whole.
...
### `ledger.corp_card_expand` — Corporation ledger, the Balance card's title row

**Press.** Click the full-canvas control on the Balance card's title row

**Valid when:**
- Corporation ledger is open

**Expected output.** Gives the Balance card the canvas region: the same two-column chart (earnings against stacked expenses) at screen size, with the ledger column still visible beside it. Only one thing can be expanded at a time. Clicking the return control top-left (or pressing Esc) folds it back. There is no expand-in-place control on this card - it already shows its chart at rest, so the gutter's left slot is empty.

**Reason to select.** The column chart is legible at rest; the takeover is for reading the segment proportions and hovering individual flows with room to work.

### `ledger.corp_balance_segment_hover` — Corporation ledger, a segment of the Balance card's expense column

**Press.** Hover a segment of the stacked Expenses column
...
**Expected output.** Toggles the Corporation ledger open in the fold-out column. Opening it closes whichever other ledger was open (accordion). If it is already open, clicking again closes it (toggle rule). Open, it shows ONE card, Balance: a verdict line ('+X/qtr, balance Y') over a two-column chart - the quarter's earnings against its expenses stacked by flow - and the quarter's net beneath it. Production and Workforce are answered by the Construction ledger's Buildings tab, Trade by the Market and Convoys ledgers.

**Reason to select.** Answers 'how well am I doing?' - the money read, which is the honest answer to that question today.
...
### `chrome.epoch_cli` — Command line. The only route to a start other than the default 1960 one (BL-705 opened the flag; the 0 CE start remains supported).
...
**Expected output.** Sets world_params::epoch_year — the calendar the clock rebases to at Begin — and nothing else. Default 1960. Generation runs the history 2400 BCE → 1960 whatever the epoch (the migration, the Empires, the Exploration and Industrialisation spans on one engine), so every epoch builds the same world; the flag names the date it is met on. The recipe band is NOT the epoch's: it is derived from the history's industry state at the 1960 fold — industrial iff any living polity's materials capacity reaches the industrial rung, ancient otherwise (docs/economy/ERAS.md § Where the ladder starts) — persisted on the world and applied on a new game and a loaded save alike. Epoch 0 is retired (Ben, 2026-09-25, NR-920): no band override and no ancient-roster sandbox; an ancient opening exists only where a world's history never crossed the rung (docs/generation/INDUSTRIALISATION.md). The UI calendar follows it: ui::fmt::campaign_epoch_year() is published from the live world_params, so a 1960 campaign renders 1960-based dates and a 0 CE campaign renders 0 CE ones. That is display only and never enters `world` — the serialised history datum (::history_datum_year) is a separate fixed constant and does not move with the epoch.
