...
**"What's my move on the thing I just clicked?"** This is the opposite of a ledger: not exhaustive reference reached from the rail, but a decision prompt that appears *because* the player selected something. It is polymorphic by selection **kind** (`selection_kind_of`: tile / body / building / market / unit / nation / corporation, plus a **province** and an **active battle**, which are not entities but borrow the band). Each kind leads with its **action**, backed by decision-relevant **facts**, plus a `[>]` 'go to' (`focus_on_entity`) into the deep ledger where the encyclopedic detail lives. Secondary question per kind: *"what do I need to know to make that move?"* — the tile's suitability, the building's profitability, the body's commercial pulse, the unit's strength.

## 2. Sub-levels — views & default
Not tabbed views — **polymorphic by selection kind**. Two layout families share the band: the **three-column card** (left quarter: a picture; centre: a paged accordion of views with a ‹ Name (i/N) › pager and the `disclosure_controls` full-canvas hook; right quarter: a 2×3 action-glyph grid), and the older **action | facts split** (`draw_selection_action` at a 58 % width, `draw_selection_facts` to its right) for the kinds that have not had their own mockup.

| Kind | Layout | Action | Facts / views | 'Go to' target |
|---|---|---|---|---|
| **Tile** | three-column card, own header (`draw_tile_selection`) | **Construct Buildings** (opens the tile construction ledger — extractable-target picker, cost-annotated build buttons, affordability-gated), plus the rest of the glyph grid | Terrain (habitability / hazard vs body average) · Resources (per-deposit production vs top-decile, dropdown-selected, click-drillable to history) · Available buildings (per **province**: count vs ceiling, Built / Max per workable resource) | — (already on the surface) |
| **Province** | three-column card under the shared header (`draw_province_selection_body`) | — | the province in situ over its terrain mixture bar; title `Province [x, y]` from its anchor tile, `N tiles` | anchor tile |
...
**Cross-cutting selectors (NOT views, toggle-exempt):** the Resources view's **deposit dropdown** (`ui_state::card_resource_page`) and the tile construction ledger's **extractable-target** picker — each picks *what*, not a view. The accordion pagers (`card_tile_view`, `selection_building_page`, `selection_unit_page`) are view selectors inside the card, not ledger tabs.
...
- Terrain and Resources views: `tile_component` habitability / hazard / `resource_deposit`, `placement_rules::k_extractable`, the body-wide top-decile reference; Available buildings: `stack_capacity` summed over province member tiles through `can_place_in_world`.
...
- **Re-populates** on a valid single-click selection (`selected_entity` set, `selection_kind != none`); a province press sets `selected_province`, a battle press the `selected_battle_*` triple.
...
- **Do the action | facts kinds (body, market, nation, corporation) migrate to the three-column card?** Tile, building, unit, province and battle have their own mockups; the rest keep the 58/42 split until they do. Confirm the card is the target shape for all kinds.
