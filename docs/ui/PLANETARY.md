...
| Road network | **Always-on** (like terrain, not a lens): the generated road lattice (BL-146) plus player-placed roads (BL-147) render as edges between adjacent roaded tiles — each roaded tile draws a segment to its roaded right + down cardinal neighbour (so each edge draws once; a cylinder-seam edge is shifted one period to stay short). **Trunk** roads (`road_level` 2, the generated backbone between major centres) draw thicker/brighter; **local** roads (`road_level` 1, the tier the player places) thinner/dimmer. Drawn only into survey-revealed neighbours, so roads don't leak past the survey fog. |
