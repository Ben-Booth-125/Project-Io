...
Wired to the live economy as of the Layer 3 finalisation (`src/ui/header_panel.{hpp,cpp}`). The header design pass settled it as a **summary surface**: three money figures plus a trend line, with all detail left to the ledgers. BL-177 (runway read, 2026-07-30) and BL-073 (debt flag) have since added two survival signals to the same row — see § Contents.
...
1. **Runway ladder (BL-177)** — resolved first, in three honest steps:
...
This ladder is the **reference case for BL-215** (text-wrap render audit — open, designed): the
audit's job is to make every other worded surface degrade this honestly rather than wrap or clip.
