...
- Fix the instrument first (BL-554), re-measure the GENERATED era, and only then choose a fix - the honest order, and the one that avoids tuning a scorer weight on a number from a simulation the game does not run.
