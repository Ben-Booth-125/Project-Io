...
> **RESOLVED.** OVERTURNED. Ben, 2026-09-02, via the review form: ONE price table; shorten the ancient chain to depth 1. Applied: the base_price_ancient table, the world_gen_config override and base_price_for_epoch are removed (era_band.hpp stays - a harmless header split); recipes.lua gains steel_bloomery (Bloomery Furnace: iron_ore 2 + timber 1.5 -> steel, ancient, appended), the Smithy becomes the deeper alternate at 0.4 blooms + 0.2 charcoal; the single table re-derived with each good at the larger of the two bands' needs (steel 16.1, machinery 60.8, alloys 84.8, consumer_goods 60.8, ordnance 155.0, spacecraft_components 308.4, construction_capacity 6.6; ladder ~123x).
...
> **RESOLVED.** RATIFIED. Ben, 2026-09-02, via the review form: all three stand - processing maintenance 10 -> 2, extraction base_rate 20 -> 40, regolith 0.6 -> 1.0.
...
> **RESOLVED.** RATIFIED. Ben, 2026-09-02, via the review form: k = 1.0 on the cheapest in-band route, k = 0 plus the floor half on every alternate.
