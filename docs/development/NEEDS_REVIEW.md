...
> **RESOLVED.** RULED by Ben, 2026-09-08, and the ruling went past all three options offered. First: a region emptied by war is immediately resettled by the attacker. Then, decisively: POPULATION IS CIVILIAN, ARMIES ARE DISTINCT FROM IT, AND STAGE 4 DOES NOT SIMULATE TOTAL WARFARE - so war stops producing empty regions at all, and the pathology has no cause rather than a block. That also dissolves the assimilation-bypass worry I raised against the first answer: with no war route to emptiness there is no massacre-and-resettle fast path. Written into GENERATION_STRATEGY.md § Population is civilian, and armies are distinct from it. Work is BL-835, re-authored around the split.
