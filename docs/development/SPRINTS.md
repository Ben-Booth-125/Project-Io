...
**Goal.** Make the rival play like a person rather than an optimiser, and BE ABLE TO WATCH IT DO SO. The design turned over on day one and the new shape is the point: no corporation holds itself back, because a handicap in the scorer is invisible, uncausable and uncounterable. Instead LEADING IS EXPENSIVE - coalitions form against whoever leads, and (later) a strained climate bears hardest on the largest operator. The slim margin becomes an emergent property of a healthy world rather than a target any actor aims at. The sprint builds the instruments (a decision feed that reads, a spectate route, a composite standing index, a harness that bands the spread), then the coalition brake, and DESIGNS climate without building it. Ben, 2026-08-31: "hopefully by the end of this sprint we can use a spectator mode to view AI playing in real time, and tighten core gameplay loops."
