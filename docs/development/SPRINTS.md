...
**Sprint-number ceiling advanced to 22 (Ben, 2026-08-27: 'move sprint 21 to 22, and call our work sprint 21').** The 2026-08-24 ceiling stands in spirit - author no sprint past the one Ben opens - with 22 now the horizon. Two sprints open (21 UI visibility, 22 demand), inside the three-sprint cap.

**A renumber is NOT the delete-unstarted rule (2026-08-27).** The 2026-08-24 policy deletes sprints that never executed; the demand sprint had LANDED WAVE 0 (BL-648, BL-649), so it was moved whole to 22 under Ben's explicit condition - 'sprint 22 is important, not worth losing it'. Forward-looking references were renumbered (sprints.json, DEVLOG heading, REFINED.md, backlog.json _note, NEEDS_REVIEW sources, the verifier-headless skill). The `sprint-21-wave-0` requirements batch key was LEFT AS WRITTEN: it is an identifier two verified requirement records point at, and rewriting a key to match a later rename buys tidiness at the cost of a broken link. Read it as 'the demand sprint, before the rename'.

## Open now

### Sprint 21 — UI visibility pass
...
| 21 | UI visibility pass | Opened 2026-08-27 - a sweep over the UI element catalogue; scan coverage first, then change elements across several sessions |
| 22 | The other half of the economy - demand | Opened 2026-08-26 as Sprint 21, renumbered to 22 on 2026-08-27 - every good gets a buyer that is a mechanism, then viability is measured over repeated passes |

**Next up.** TWO sprints open. Sprint 21 (UI visibility) is newly opened and deliberately empty below wave 0 - the coverage scan runs first, then Ben picks the elements; it is expected to span several sessions. Sprint 22 (demand, opened as 21) has wave 0 landed: BL-648's guard is RED on eleven goods by design and turns green as channels land, BL-649's census is the before/after instrument. BL-654 (a short pool buys up to a reservation ceiling) gates BL-641's rates going non-zero and should land early. Sprint 20's slipped items carry forward; three are design-owed on Ben. UNRESOLVED: NR-663's family, BL-639 and NR-686 were called a 'visibility pass' on 2026-08-26 when that phrase meant the demand sprint - they are verification visibility, and which sprint owns them is Ben's call.
