> **ARCHIVED 2026-08-24.** This is the roadmap as it stood at 1,336 lines, before the trim
> Ben asked for. It is kept for one reason: the **done-definitions written at each cut** are the
> record NR-103 asked for, and the per-minor cut notes are the only prose account of what several
> minors actually found. Nothing here is live. The live roadmap is
> [`../ROADMAP.md`](../ROADMAP.md); what was built and when is `DEVLOG.md` and `SPRINTS.md`.
