...
some culture**, and then it stops. What follows is a *separate* round with a separate span: **0 CE to 1200 CE**, in which polities
contest what migration left (`CIVILISATION.md` § The span is 0 CE to 1200 CE).

**A COAST JOINS THE TWO (Ben, 2026-09-09 sets the boundary; the coast is Claude's reading,
NR-818).** This round's end is derived and the next round's start is stated, so they are not the
same year and are not meant to be. When the filling finishes, the world holds what migration left
it until 0 CE — the same device the design uses across 1200 → 1560. Nothing is simulated in
between, and nothing is reset.
