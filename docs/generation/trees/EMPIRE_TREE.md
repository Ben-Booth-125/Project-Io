...
> the situation in which a polity pursues it · what the tree hands the Globalisation tree.
...
rim milestone is the reason a polity enters the Globalisation tree early or late, which is where the
...
| EM-SP-4m | **The Enforceable Promise** | milestone | 4 | SP-4a | — | — | opens the Globalisation tree | *spire* — requires PE-4a, RD-4b |
...
by running one wedge to the rim. The rim milestone is what the Globalisation tree's root requires.
...
the Globalisation tree with *Chartered Companies* and *Ocean-Rated Hulls* and nothing to smelt.
...
## What the tree hands the Globalisation tree

- **Entry timing.** A polity holding *The Enforceable Promise* at 1200 CE starts the
  **Globalisation** tree at its root the moment that phase opens — which is 1200 CE itself, with
  the coast retired (`../GLOBALISATION.md`); one that does not starts once it holds the rim. This
  is the first input to the 1960 spread, and it is now two trees away rather than one.
- **Sea legs.** Held *Deep-Hull Sail* / *Lateen* / *Ocean-Rated Hulls* are what Globalisation's
...
- **Corporate soil.** *Chartered Companies* is the institutional half of what the Industry tree's
  ...
  there at 1960 — reached through Globalisation, which is where a charter first has a treasury
  behind it.
