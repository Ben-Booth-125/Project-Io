...
> domestication package · one branch per kind of ground and what that ground teaches · the three
...
ground a seat can stand on. One major and one milestone per ring, chained; the four branches leave
the spire at its ring-1 major.

| id | node | kind | ring | links | gate | diffusion | effect | earth_ref |
|---|---|---|---|---|---|---|---|---|
| CO-SP-1a | Seed Selection | major | 1 | CO-SP-1m; the four ring-1 branch majors | — | practice | carrying_capacity +100‰ | the Neolithic package's first act — keeping the best seed back, ~9th millennium equivalent |
...
Each milestone requires the spire major at its ring **and** any two of the four branch majors at
its ring — `requires` for the first, `requires_any` for the second (`TREES.md` § Milestones, and
how a tree unlocks the next). Rule 4's breadth holds without naming which breadth. A people coined on one class and never in contact
...
|---|---|---|---|
| CO-SP-1m | The Fed Band | A people that eats more than it gathers, and has learned a second ground. | CO-SP-1a and any two of CO-FL-1a, CO-HL-1a, CO-GR-1a, CO-CS-1a |
| CO-SP-2m | The Settled Village | A store worth staying beside, on ground that keeps two kinds of harvest. | CO-SP-2a and any two of CO-FL-2a, CO-HL-2a, CO-GR-2a, CO-CS-2a |
| CO-SP-3m | The Standing Seat | A town whose common store feeds hands it does not need in the field — ground a seat can stand on. | CO-SP-3a and any two of CO-FL-3a, CO-HL-3a, CO-GR-3a, CO-CS-3a |
...
| `../COLONISATION.md` § The stream — what moves, and what it costs | The two `access` effects and the capacity modifiers | The walk prices a range or a strait cheaper for a people that carries the crossing, and a region's asymptote rises with what its people know |

---

## Open questions

- **Which of the twelve farm classes each branch owns.** The migration's classifier names twelve
  classes — boreal, volcanic, floodplain, montane, coastal, woodland, arid, valley, highland,
  grassland, stone, steppe — and `../COLONISATION.md` names none. The four branches here are the
  four families the shape was sized for; floodplain and valley read as Wet Ground, highland,
  montane and volcanic as High Ground, grassland and steppe as Open Ground, coastal as The Shore.
  Woodland, boreal, arid and stone belong to no branch, and whether they fold into one, teach
  nothing, or want a fifth branch is a call this document does not make.
- **A highland gate atom.** The gate vocabulary is `ore_q · fuel · arable · coastal · grassland`,
  and none of them says *high ground*. High Ground's majors carry no gate, so any people can hold
  the terrace by time on any ground; a `highland` atom would close that.
- **What `access` targets name.** `mountain`, `steppe` and `strait` are written here as terrain
  classes the walk prices; whether the walk reads them as farm classes, landforms or the region
  domain is the implementation's to settle.
