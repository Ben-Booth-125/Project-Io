...
BL-459 (unit strength is a duplicate of count) is changing this to a derived value. Until it lands,
`strength` carries no information `count` does not.

**2. `corporation_component::military_points` is written and read by nothing.** Every completed
`military_base` credits it a flat `military_points_per_base_tick` (default 1.0) each economy tick,
symmetrically across player, rival and background corps (BL-332, military points and research).

No spend mechanism reads it. BL-087's constellation was the intended sink and is not built.
BL-455 is deleting the accumulator; treat it as scheduled for removal, not as a resource to design
against.
...
completed base credits `military_points` at the authored rate, a completed research institute
...
- Passive `military_points` accumulation (BL-332, military points and research) — **scheduled for deletion**, see § Two live inconsistencies
