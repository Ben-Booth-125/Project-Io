...
Every quantity in Io is held by an institution. A corporation has a balance, a nation has a
treasury, a unit has a strength. **Nobody holds anything.**
...
institution to another; lobbying (BL-539, lobbying) is a corporation influencing a nation. Both are
correct and both are **abstract in a way that makes them hard to care about.**
...
tongues. A person's name is one more draw from that machinery.
...
- **BL-370 (corp leader figure)** — from Ben's 2026-08-11 notes session: a named figure standing in
  for the player's corp, *"so the player feels a personal connection to the corp they play as."*
  That is **one role**, and it is the role this system ships first.
- **BL-207 (persona counsel packs)** — advisory personas seated per corporation, reading the
  blackboard, biasing the strategy weight through a bounded, transparent nudge. **That is already a
  person-with-a-role**, fully designed, deterministic, and moddable in Lua. It is the strongest
  precedent in the repo and this design extends it rather than inventing a rival.
...
2. **A holder for doctrine.** A campaign battle takes a `doctrine_row` per side, and the row the
   campaign layer supplies is all-zero (MILITARY.md). A commander is the natural thing for a doctrine
   to belong to.
...
### A person is a name, a role, a seat, a tenure, and a legible set of biases
    ...
    person {
        ...
        biases      several, each of which must gate or bias something real
    }

**Several biases, legibly.** People are **a real optimisation axis** (Ben, 2026-08-22), so a person
carries more than one modifier. The guard against a person becoming a spreadsheet with a name is
not scarcity of modifiers but **legibility of the set**: a person is a build choice, and a build
choice you cannot read is a spreadsheet whatever its length. Each bias is expressible as a
`scalar_modifier` ([`META_LAYER.md`](META_LAYER.md)) so it inherits determinism, serialisation and
ledger legibility for free — and so **a person costs an enumerator, not a subsystem.**

### Five roles, in order, each mapped to machinery that exists

All five, in this order (Ben, 2026-08-22): company head → counsel → commander → minister → contact.

| Role | Seat | Biases / gates | Precedent |
|---|---|---|---|
| **Company head** | the player's corp | attachment first; biases second | **BL-370** — the first role |
| **Counsel** | any corp | the strategy weight, boundedly | **BL-207**, fully designed |
| **Commander** | a unit | doctrine — the `doctrine_row` a campaign battle reads | MILITARY.md § Doctrine |
| **Minister** | a nation | which priority line BL-537's budget favours | BL-537 (national budget), BL-539 (lobbying) |
| **Contact** | a (buyer, supplier) pair | who the sentiment is *with* | RELATIONS.md, BL-546 (reputation as a sentiment view) |
...
**The player's own figure is chosen at campaign setup, biases included** (Ben, 2026-08-22). The
company head is a build decision rather than a portrait, which makes it a New World wizard concern
...
> **A cast, not a population.** People are generated only where a role is *seated*, and roles are
> seated sparsely — the player's own corp, nations the player has met, units the player commands,
> suppliers the player has dealt with. **A person is created when someone would plausibly know
> their name**, which makes the discovery layer (BL-089's activity fog) the natural gate and costs
> nothing to generate.

**A rival's people are banded like everything else about it** (Ben, 2026-08-22; BL-068,
competitor visibility). You learn who they are as you learn the rest of it — no special case, and
no new machinery for the visibility question.
...
**People are mortal** (Ben, 2026-08-22): they age on the economy tick, leave their seat
deterministically, and a successor is drawn with different biases. That gives the game a source of
**change the player did not cause and cannot prevent** — which is the same argument BL-289 (sky
events) makes for a supernova, at a much smaller scale and much more often.

Succession is also the cleanest first **event** ([`EVENTS.md`](EVENTS.md)), which is why the two
documents were written together — and since Ben chose *personal* as the first event family, **this
system gates that one.**

### The pre-history

**A few people exist in the pre-history** (Ben, 2026-08-22) — only the figures whose deeds the
campaign world still remembers. `history_log` records deeds with nobody to attribute them to; this
names the memorable ones and leaves the rest institutional.
...
- **A person must not decorate.** If a role's biases were removed and nothing measurable changed,
  the role should not ship.
...
guard against by other means: legibility of the set, not scarcity of modifiers.

**People are MORTAL.** Age and replacement, deterministically.

**The player's own figure is CHOSEN AT START, biases included.**

**Banded like everything else** (BL-068).

**A few exist in the pre-history** — only the figures whose deeds the campaign world still
remembers.

**PEOPLE gates EVENTS.** Ben chose *personal* as the first event family
([`EVENTS.md`](EVENTS.md)), and that family depends on this one. What ranked fourth by available
machinery is first by ruling, so this system sits **ahead** of the event system rather than beside it.
...
1. **Which roles, and in what order?** Five are proposed. **Company head (BL-370) is the obvious
   first** because it is Ben's own ask and needs no other system. **Minister is the highest-value**
   because it gives lobbying a target, but it depends on BL-537's budget.
...
| Doctrine, which a commander supplies | `src/world/combat.cpp`, `campaign_battle.cpp` |
| The player's figure at setup | the New World wizard, `ui/STARTUP.md` |

**Related authorities.** [`EVENTS.md`](EVENTS.md) (succession is the first event),
[`politics/RELATIONS.md`](politics/RELATIONS.md) (the sentiment a person holds),
[`politics/NATIONS.md`](politics/NATIONS.md) (the budget a minister weights),
...
precedent every other role follows.
