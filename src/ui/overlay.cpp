...
{
    // The on-screen lenses, in settled order (BL-013, trimmed BL-093/BL-604):
    // Corp → Country → Resource → Market → Population → Continent.
    ...
    // BL-604 retired Opportunity and Production, so the row is six wide; the array
    // extent is deduced rather than written, which is what kept the previous two
    // trims from leaving a stale literal behind.
