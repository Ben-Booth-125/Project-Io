...
struct nation_component
{
    ...
    /// The nation's own money (BL-480 / the D4 tariff). A nation had no balance
    /// at all until a law of its own could collect one: an import tariff enacted
    /// by this nation credits HERE, exactly the sum the buying corporation is
    /// debited, so the flow is a transfer and not a mint. Zero at generation —
    /// nothing else spends it yet, and a treasury that started full would be a
    /// balance change smuggled in as a field.
    float treasury = 0.0f;
};
