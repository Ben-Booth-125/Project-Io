...
{
    const float base = (tc.substrate == terrain_substrate::ocean)
                           ? sea_leg_cost
                                      ...
                                      entity_id dst_tile)
{
    ...
    if (src_tile == dst_tile)
    {
        res.reachable     = true;
        res.cost          = 0.0f;
        res.crosses_ocean = (sit->second.substrate == terrain_substrate::ocean);
        res.tiles         = { src_tile };
        return w.astar_cost_cache.emplace(key, std::move(res)).first->second;
    }
    ...
    crossed[static_cast<std::size_t>(start)] =
        (sit->second.substrate == terrain_substrate::ocean) ? 1 : 0;
            ...
            if (nd < dist[static_cast<std::size_t>(nidx)])
            {
                ...
                crossed[static_cast<std::size_t>(nidx)] =
                    (crossed[static_cast<std::size_t>(idx)]
                     || n_tc->substrate == terrain_substrate::ocean) ? 1 : 0;
