        ...
        for (polity& q : out.polities)
        {
            ...
            for (int hi : held)
            {
                for (int tn : neighbours[static_cast<std::size_t>(hi)])
                {
                    ...
                    // THE HUB'S OWN WORKS DISCOUNT THE TERRAIN COST (BL-321).
                    // reach_mod is authored as "a discount on the supply cost
                    // through/from this province", and `hi` is the province the
                    // campaign is staged from — so it is that province's roads
                    // and wharves that are doing the carrying, not the
                    // capital's. Applied to the terrain term alone: a Span
                    // Bridge makes a mountain cheaper to cross; it does not
                    // shorten the march, which is what supply_decay_per_tile_q
                    // charges for.
                    const int hub_reach_q = clampi(
                        ss.provinces[static_cast<std::size_t>(hi)].work_reach_mod,
            ...
            case sim_verb::campaign:
            {
                ...
                // The staging province's works discount the terrain term here
                // exactly as they did in the scorer (BL-321) — the two must
                // agree, or a polity would decide on one supply figure and
                // fight on another.
                const int home_reach_q = clampi(home.work_reach_mod, 0,
                                                params.work_reach_relief_cap_q);
                const int reach_charge = reach_cost * params.terrain_reach_cost_q / 100;

                const int supply_raw = 1000
                                     - cap_dist * params.supply_decay_per_tile_q
                                     - (reach_charge - (reach_charge * home_reach_q) / 1000)
                                     - clampi(burden, 0, 1000 - params.holdings_burden_floor_q);
